...
def main():
    ...
    for test in tests:
        ...
        cur = dbconn.cursor()
        cur.execute('''select testrun_id, timestamp, identifier, returncode, key, value, unit
                       from result, testrun 
        ...
        overview.append("{0:30} \t [{1}]".format(test.name, ["FAILED", "PASSED"][result[0][3]]))
        
        text.append("\n")
        text.append("===== {0:60} =====".format(test.name,))
